...
def  linear_decay(steps):
    n_epochs = 600
    return 0.2 + (0.4/n_epochs) * steps

def exp_decay(steps):
    n_epochs = 600
    return 0.2 * math.e**(math.log(3)/n_epochs * steps)

def cos_decay(steps):
    n_epochs = 600
    return 0.2 + math.cos(math.pi/2 - math.pi/(2*n_epochs) * steps) * 0.4

def cvi(env_fn, actor_critic=core.mlp_actor_critic, ac_kwargs=dict(), seed=0, 
        steps_per_epoch=5000, epochs=100, replay_size=int(1e6), gamma=0.99, alp = 0.8,
        polyak=0.995, lr=1e-3, alpha=0.2, batch_size=100, start_steps=10000, 
        max_ep_len=1000, logger_kwargs=dict(), save_freq=1, decay = None, squash = False, hard = 1000):
    ...
    with tf.variable_scope('target'):
        _, _, _, q1_targ, q2_targ, _, _, v_targ  = actor_critic(x2_ph, a_ph, **ac_kwargs)
    ...
    with tf.control_dependencies([train_value_op]):
        ...
        ##Do hard updates
        #target_update = tf.group([tf.assign(v_targ, tf.cond(tf.not_equal(t_step%hard,0), lambda: v_targ, lambda: v_main))
                                  #for v_main, v_targ in zip(get_vars('main') , get_vars('target'))])
